...
function sendTelegramLog(message) {
    // Убираем HTML теги если они есть, чтобы не ломать парсинг ТГ (простая очистка)
    ...
    https.get(url, (res) => {
        // Ответ от ТГ не обрабатываем, чтобы не засорять логи, главное отправили
    }).on('error', (e) => {
...
function startBot(config, index) {
    ...
    const options = {
        ...
        offline: true,
        conLog: null, // Отключаем встроенный лог bedrock-protocol, чтобы писать свой
        connectTimeout: 20000,
        skipPing: true // Иногда помогает ускорить подключение
    };
    ...
    const scheduleReconnect = (reason) => {
        ...
        // Логируем причину и уведомляем в ТГ
    ...
    client.on('error', (err) => {
        // Не вызываем scheduleReconnect здесь напрямую, так как часто за 'error' следует 'close' или 'end'
        // Но на всякий случай логируем. Основной реконнект сработает на 'end' или 'close'
        console.error(`[Bot #${botId}] Внутренняя ошибка сокета: ${err.message}`);
    });

    client.on('kick', (reason) => {
        scheduleReconnect(`Кикнут сервером. Сообщение: ${reason}`);
    });
        ...
        try {
            ...
            setTimeout(() => {
                if (!isReconnecting && client) {
                    ...
                    // Не спамим в ТГ вводом пароля, только в консоль для отладки если надо
                    // console.log(`[Bot #${botId}] Пароль отправлен.`);
                }
            }, 2000);
        } catch (e) {
    ...
    client.on('text', (packet) => {
        ...
        // ChatLog сохраняем только для веб-сайта, в Телеграм весь чат не шлем (иначе будет спам)
        const msgObj = {
            id: botId,
...
// Защита от падения всего скрипта при необработанных ошибках
process.on('uncaughtException', (err) => {
    console.error(`[CRITICAL] Необработанная ошибка: ${err.message}`);
    // Не выходим из процесса, позволяем ботам перезайти
});
